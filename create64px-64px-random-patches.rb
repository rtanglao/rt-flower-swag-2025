...
if ARGV.length < 2
  puts "usage: #{$PROGRAM_NAME} [filenames-of-3424x3424px originals] [number of 64px_64x patches]"
  exit
end
...
number_of_patches.times do |_i|
  ...
  x_offset = rand(MAX_X_INDEX) * 64
  y_offset = rand(MAX_Y_INDEX) * 64
  ap file
  filebasename = File.basename(file, '.jpg')
  filename = format(
    '%<basename>s-64x64patch-%<x_offset>4.4d-%<y_offset>4.4d.png',
